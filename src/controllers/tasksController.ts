  ...
  try {
    ...
    if (!title || position === undefined || column_id === undefined) {
      return res.status(400).json({
        success: false,
        message:
          "Tasks must contain a title, description, position and column_id",
      });
    }
...
export const updateTaskByUserController = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const updated = await updateTaskByUserModel(
      title,
      description,
      position,
      column_id,
...
export const deleteTaskByUserController = async (
  req: Request,
  res: Response,
) => {
  try {
